...
############################################
#####    chromoGenome: constructor    ###### 
############################################

chromoGenome <- function(
    inp_annot = "biomart", # c("biomart", "annotated", "gtf") 
    dataset = "hsapiens_gene_ensembl", 
    ensembl = NULL,
    gtf
  ){
  
  if(inp_annot == "biomart"){
    if(is.null(ensembl)){
      ensembl <- useMart("ensembl", dataset = dataset) # connection to ENSEMBL
    }
    chromoObject <- new("Chromo", genome = list(inp_annot = inp_annot, dataset = dataset, ensembl = ensembl))
  }else if(inp_annot == "annotated"){
    #nothing
    chromoObject <- new("Chromo", genome = list(inp_annot = inp_annot))
  }else if(inp_annot == "gtf"){
    #gtf processing and cleaning first?
    chromoObject <- new("Chromo", genome = list(inp_annot = inp_annot, gtf = gtf))
  }
  
  # include cytobands df
...
chromoLocate <- function(
    chromoObject,
    DEdf,
    pval_cutoff = 0.05, 
    log2fc_cutoff = 1,
    # if providing an already anoted df:
    gene_col, 
    fc_col, 
    p_col
  ){
  if(chromoObject@genome$inp_annot == "biomart"){
    all_features <- getBM(
      ...
      mart = chromoObject@genome$ensembl
    )
    ...
    chromoObject@columns <- list(gene_col = gene_col, fc_col = fc_col, p_col = p_col, 
      chromosome = "chromosome_name", start_position = "start_position", end_position = "end_position", 
      avg_position = "avg_position", gene_length = "gene_length", DEG = "DEG")
...
chromoScatter <- function(
    ...
    only_expr_features = F, #calculates percentage based on only expressed features (useful when separate_by = "celltype")
    score_method = "pct", # c("pct", "fish", "chi"), other tests?
    highlight_features = "top_each", # gene name to write
  ...
  # highlight features ####################### highlight system that when separating by cell type it doesn't repeat genes across cell types
  max_genes <- chromoObject@data %>%
    filter(DEG == "UP") %>% 
    group_by(!!sym(separate_by)) %>% 
    slice_max(order_by = !!sym(chromoObject@columns$fc_col), n = 1, with_ties = FALSE) %>%
    pull(!!sym(chromoObject@columns$gene_col))
  min_genes <- chromoObject@data %>%
    filter(DEG == "DOWN") %>% 
    group_by(!!sym(separate_by)) %>%
    slice_min(order_by = !!sym(chromoObject@columns$fc_col), n = 1, with_ties = FALSE) %>%
    pull(!!sym(chromoObject@columns$gene_col))
  
  if(highlight_features == "top_each"){
    aux <- c(max_genes, min_genes)
  }else{
  ...
  local_aux <- chromoObject@data %>% 
    mutate(
      ...
      color = case_when(
        !is.na(highlight) & DEG == "UP" ~ color_text_up,
        !is.na(highlight) & DEG == "DOWN" ~ color_text_down,
        TRUE ~ NA
      )
    )
