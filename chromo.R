...
chromoInitiate <- function(
    ...
    if(inp_annot == "default_human"){
      ...
      chromoObject <- new("Chromo", genome = list(inp_annot = inp_annot, cytobands = cytobands))
...
chromoDensity <- function(
  ...
  calculate_density <- function(subset, chro) { # is the really necessary?
    dens <- density(
      subset[[avg_position]],
      #n = 2^ceiling(log2(max(cytobands[cytobands$chr == chro, "baseEnd"]))),
      kernel = "gaussian",
      from = 1,
...
chromoDensityPlot <- function(
  ...
  for(i in 1:length(chr_with_clu)){
    ...
    plots_list[[i]] <- ggplot() +
      ...
      scale_x_continuous(expand = c(0, 0), limits = c(0, max(as.numeric(cytobands %>% filter(chr %in% chr_with_clu) %>% pull(baseEnd))))) + # remove padding to y axis and IMPORTANT fix size for all chr
    ...
    if(include_density & chr_with_clu[i] %in% top_clusters$chromosome){
      ...
      dens <- density(
        ...
        #n = 2^ceiling(log2(max(cytobands[cytobands$chr == chr_with_clu[i], "baseEnd"]))),
        kernel = "gaussian",
        from = 1,
